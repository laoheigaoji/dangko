...
export default function ProfileTurnover() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/users/${user.phone}/turnover`)
      .then(res => res.json())
      .then(data => {
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await fetch(`/api/turnover/${deleteId}`, { method: "DELETE" });
      if (res.ok) {
    ...
    return (
      <div className="min-h-screen bg-[#f5f6f8] flex items-center justify-center font-sans">
        <div className="flex flex-col items-center gap-2">
          <div className="w-8 h-8 border-4 border-gray-300 border-t-[#4b8cd9] rounded-full animate-spin"></div>
        ...
        {items.length === 0 ? (
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center px-1 mb-2">
              ...
              <button 
                onClick={() => navigate('/profile/publish-turnover')}
                className="text-[13px] text-[#4b8cd9] font-medium"
              >
                + 继续发布
              </button>
            </div>
              ...
              <div key={item._id} className="bg-white rounded-xl shadow-[0_2px_10px_rgba(0,0,0,0.03)] border border-white p-4 relative overflow-hidden group">
                ...
                <div className="flex gap-4 mb-3">
                  <div className="flex-1 bg-gray-50 rounded-lg p-2.5">
                    <p className="text-[10px] text-gray-400 mb-0.5">转让金额</p>
                    <p className="text-[15px] font-bold text-gray-700">¥{item.originalPrice}</p>
                  </div>
                  <div className="flex-1 bg-[#f0f7fe] rounded-lg p-2.5">
                    <p className="text-[10px] text-[#4b8cd9] mb-0.5">转让价格</p>
                    <p className="text-[15px] font-bold text-[#4b8cd9]">¥{item.transferPrice}</p>
                  </div>
                </div>

                <div className="flex justify-between items-center text-[12px]">
                  <div className="text-gray-500 bg-gray-100 px-2 py-0.5 rounded">
                    折扣: {(item.transferPrice / item.originalPrice * 10).toFixed(1)}折
                  </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-6">
          ...
          <div className="bg-white rounded-2xl w-full max-w-sm relative z-10 overflow-hidden shadow-2xl animate-in zoom-in duration-200">
            <div className="p-6 text-center">
              ...
              <p className="text-sm text-gray-500">删除后将无法恢复，确定要移除这条转让信息吗？</p>
            </div>
